...
class Player_data:
    """Класс создан для взаимодействия с хранилищем данных для получения данных о пользователе
        ...
        1)Имеет методы
            getinfo(считавания данных Пользователя С хранилище данных)
            setinfo(запись данных Пользователя в хранилище)
            standart_value(Выставления стандартных аргументов для пользователя)
        
        2)Имеет параметры
            amount(int),currency(str),profile(str),change_value(int(bool)),chg_amount(int(bool))
